...
export function BentoCard({ card, className, sizes }: BentoCardProps) {
  ...
  return (
    <article className={`relative isolate min-h-0 overflow-hidden rounded-[20px] bg-stone-200 ${className}`}>
      <Image
        alt={card.title}
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-3 p-4 text-white sm:p-5">
        ...
        {visibleTags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {visibleTags.map((tag) => (
              <span className="rounded-full bg-white/20 px-2.5 py-1 text-[11px] font-medium leading-none backdrop-blur-sm" key={tag}>
                {tag}
              </span>
            ))}
