...
async function loadBentoData(): Promise<BentoData> {
  ...
  const [settings] = await db.select({ layoutBentoBox: siteSettings.layoutBentoBox })
    .from(siteSettings)
  ...
  const artworkIds = [...new Set(Object.values(normalized).flat().map((item) => item.artPieceId))];
  const mediaIds = [...new Set(Object.values(normalized).flat().map((item) => item.mediaId))];

  if (artworkIds.length === 0 || mediaIds.length === 0) return { desktop: [], mobile: [] };

  const [artworkRows, mediaRows, tagRows] = await Promise.all([
    db.select({ artPieceId: artPieces.artPieceId, title: artPieces.titleEng, year: artPieces.yearOfExecution })
      .from(artPieces)
      .where(and(inArray(artPieces.artPieceId, artworkIds), eq(artPieces.isVisible, true))),
    db.select({ mediaId: media.mediaId, artPieceId: media.artPieceId, fileType: media.fileType })
      .from(media)
      .where(inArray(media.mediaId, mediaIds)),
    db.select({ artPieceId: artPieceTags.artPieceId, name: tags.nameEng })
      .from(artPieceTags)
  ...
  for (const tag of tagRows) {
    if (tag.name) tagsByArtwork.set(tag.artPieceId, [...(tagsByArtwork.get(tag.artPieceId) ?? []), tag.name]);
  }

  const cardsFor = (name: LayoutName) => normalized[name].flatMap((item) => {
    const artwork = artworksById.get(item.artPieceId);
    const selectedMedia = mediaById.get(item.mediaId);
    if (!artwork || !selectedMedia || selectedMedia.artPieceId !== item.artPieceId || !isBentoMediaType(selectedMedia.fileType)) {
      return [];
    }
